...
import in.dailyhunt.avro.io.parsing.Parser;
import in.dailyhunt.avro.io.parsing.Symbol;
import in.dailyhunt.avro.io.parsing.ValidatingGrammarGenerator;
...
public class ValidatingDecoder extends ParsingDecoder
  ...
  @Override
  public long readOptionalStart() throws IOException {
    parser.advance(Symbol.OPTIONAL_START);
    long result = in.readOptionalStart();
    if (result == 0) {
      // pop symbol in case optional is not present
      parser.popSymbol();
      parser.advance(Symbol.OPTIONAL_END);
    }
    return result;
  }

  @Override
  public void markOptionalEnd() throws IOException {
    parser.processTrailingImplicitActions();
    parser.advance(Symbol.OPTIONAL_END);
  }
