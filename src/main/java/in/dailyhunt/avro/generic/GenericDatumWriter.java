...
import in.dailyhunt.avro.AvroTypeException;
...
public class GenericDatumWriter<D> implements DatumWriter<D> {
  ...
  protected void writeWithoutConversion(Schema schema, Object datum, Encoder out)
    ...
    try {
      ...
      case SET:    writeSet(schema, datum, out);    break;
      ...
      case LRU_SET: writeLruSet(datum, out);          break;
      case OPTIONAL: writeOptional(schema,datum, out);break;
  ...
  private void writeSet(Schema schema, Object datum, Encoder out) throws IOException {
    final Schema schemaElementType = schema.getElementType();
    final Set set = (Set) datum;
    long size = set.size();
    long actualSize = 0;
    ...
    for(Object item : set){
      out.startItem();
      write(schemaElementType,item,out);
      actualSize++;
    }
  ...
  private void writeLruSet(Object datum, Encoder out) throws IOException {
    int size = getMapSize(datum);
    int actualSize = 0;
    out.writeMapStart();
    out.setItemCount(size);
    ...
    for (Map.Entry<Object,Object> entry : getMapEntries(datum)) {
      out.startItem();
      LruValue lruValue = (LruValue) entry.getValue();
      writeString(entry.getKey().toString(), out);
      out.writeLong(lruValue.getLastAccess());
      out.writeLong(lruValue.getCount());
      actualSize++;
    }
    out.writeMapEnd();
    if (actualSize != size) {
      throw new ConcurrentModificationException("Size of LruSet written was " +
          size + ", but number of entries written was " + actualSize + ". ");
    }
  }

  private void writeOptional(Schema schema, Object datum, Encoder out) throws IOException {
    Optional optional = (Optional) datum;
    ...
    final Schema optionalValueSchema = schema.getElementType();
    out.writeLong(1);
    writeWithoutConversion(optionalValueSchema,value,out);
